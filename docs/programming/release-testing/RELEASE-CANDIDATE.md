...
- Fixed the swiper bar drifting out of alignment with the clipped layer edge on map resize: the `Draggable` `key` changed in the same render where the bar's position state was still stale, and the position was only corrected by an effect afterward — so the bar kept the old `defaultPosition` while the clip rectangle (percentage of the live map size) moved, leaving a visible gap on grow/shrink that only a manual drag fixed. The bar position is now derived directly from the divider percentage × current map size in the same render as the `key`, so the bar and the per-layer clip stay aligned at load, on grow, and on shrink (redundant position state removed) (#3625)
